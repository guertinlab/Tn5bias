...
options(scipen = 100)
#Get input variable
union_bed_loc = Sys.getenv("INPUT")
#Get unscaled read depth
print('Reading unscaled bed file...')
unscaled_bed = paste(substr(union_bed_loc, 1, nchar(union_bed_loc)-15), '_not_scaled.bed', sep = '')
unscaled_bed = fread(unscaled_bed)
unscaled_bed = sum(unscaled_bed$V5)
#Load in union bedgraph
print(paste('Reading bed file', union_bed_loc))
x <- fread(union_bed_loc)
masknames = c("XXXXXXXXXXXXXXXXXNNNNNNCNXXXXXXXXXXXXXXXXXXXXXX",
...
x[, RuleEnsemble := ((XXXXXXXXXXXXXXXXXXXXXXXCXXNNNNNNNXXXXXXXXXXXXXX*0.488249030865301) + 
...
write.table(x[,c(1:3,17)], file = paste(substr(union_bed_loc,1, nchar(union_bed_loc)-15),'_RE_scaled.bedGraph', sep = ''),
            col.names = FALSE, row.names=FALSE, sep = '\t', quote=FALSE)

#Get genome variable for chrom.sizes
chrom_sizes = Sys.getenv("GENOME")
chrom_sizes = paste(chrom_sizes, '.chrom.sizes', sep = '')

#Write scaled bigWig file
print('Writing rule ensemble scaled bigWig file...')
system(paste('bedGraphToBigWig', paste(substr(union_bed_loc,1, nchar(union_bed_loc)-15),
                                 '_RE_scaled.bedGraph', sep = ''), chrom_sizes,
      paste(substr(union_bed_loc,1, nchar(union_bed_loc)-15),'_RE_scaled.bigWig', sep = '')))
