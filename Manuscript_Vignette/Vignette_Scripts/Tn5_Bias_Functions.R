...
plot.composites <- function(dat, x_axis_range = min(dat$x):max(dat$x), ylabel = '',
                            ...
                            xlabel = '', striplabel = TRUE, legend = TRUE, labsize = 1.1, xlabsize = 1.3,
                            motifline = FALSE, Motiflen = 10, nYticks = 3, Y_ticks = TRUE, ylabsize = 1.3,
                            figwidth = 2.5, figheight=3, hline = FALSE, nYaxisdigits = 3, hlinelwd = 2,
                            indexlist = NULL, layoutgrid = NULL, y_axis = FALSE, linethick = 2, hlinelty = 2,
    ...
    print(xyplot(est ~ x|factor, group = group, data = dat, strip = striplabel,
                 ...
                 ylab = list(label = paste(ylabel), cex =ylabsize),
                 xlab = list(label = paste(xlabel), cex =xlabsize),
                 index.cond = indexlist,
                 layout = layoutgrid,
                 panel = function(x, y, ...) {
                   panel.xyplot(x, y, ...)
                   if (hline == TRUE)
                   {panel.abline(h = hline_val, lty =  hlinelty, lwd = hlinelwd, col = '#FF0000')}else{}
