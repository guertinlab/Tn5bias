...
for (i in 1:length(unique(x$Factor))) {
  RE_frame[i,1] = mean(x[which(x$Factor == unique(x$Factor)[i] & x$Treatment == 'Rule Ensemble'),1])
  RE_frame[i,2] = var(x[which(x$Factor == unique(x$Factor)[i] & x$Treatment == 'Rule Ensemble'),1])
  RE_frame[i,3] = unique(x$Factor)[i]
  seqOutBias_frame[i,1] = mean(x[which(x$Factor == unique(x$Factor)[i] & x$Treatment == 'seqOutBias'),1])
  seqOutBias_frame[i,2] = var(x[which(x$Factor == unique(x$Factor)[i] & x$Treatment == 'seqOutBias'),1])
  seqOutBias_frame[i,3] = unique(x$Factor)[i]
  unscaled_frame[i,1] = mean(x[which(x$Factor == unique(x$Factor)[i] & x$Treatment == 'Unscaled'),1])
  unscaled_frame[i,2] = var(x[which(x$Factor == unique(x$Factor)[i] & x$Treatment == 'Unscaled'),1])
  unscaled_frame[i,3] = unique(x$Factor)[i]
}
...
pdf(file = "Figure5B_TF_stats.pdf", height = 6, width = 15)
grid.table(combined_frame, rows = rep('', nrow(combined_frame)))
dev.off()

#All together stats mean
unscaled_mean = mean(x[which(x$Treatment == 'Unscaled'),1])
seqOutBias_mean = mean(x[which(x$Treatment == 'seqOutBias'),1])
RuleEnsemble_mean = mean(x[which(x$Treatment == 'Rule Ensemble'),1])
...
unscaled_var = var(x[which(x$Treatment == 'Unscaled'),1])
seqOutBias_var = var(x[which(x$Treatment == 'seqOutBias'),1])
