...
pswm.func.2 <- function(x.ligation, out = 'outfilename', posnum, rc) {
   col.matrix = matrix()
   for (g in 1:posnum){
...
dir = paste0('/home/jwolpe/Desktop/20200714_section2_3_RCR')
setwd(dir)

#
#Input filenames (from wd) into uppercasenames in this order: pe1.minus, pe1.plus, pe2.minus, pe2.plus
uppercasenames <- list('C1_gDNA_rep1_PE1_minus_not_scaled.fasta', 'C1_gDNA_rep1_PE1_plus_not_scaled.fasta', 'C1_gDNA_rep1_PE2_minus_not_scaled.fasta', 'C1_gDNA_rep1_PE2_plus_not_scaled.fasta')
...
uplist <- NULL
####Null uplist because it is big

pswm.pe1.minus = pswm.func.2(pe1.minusATAC[,1], 'ATAC_bias_pe1_minus', 20, TRUE)
pswm.pe1.plus = pswm.func.2(pe1.plusATAC[,1], 'ATAC_bias_pe1_plus', 20, FALSE)
pswm.pe2.minus = pswm.func.2(pe2.minusATAC[,1], 'ATAC_bias_pe2_minus', 20, TRUE)
pswm.pe2.plus = pswm.func.2(pe2.plusATAC[,1], 'ATAC_bias_pe2_plus', 20, FALSE)
